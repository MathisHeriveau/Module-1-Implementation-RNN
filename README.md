...
## 💻 **Implémentation du RNN : Code**

Voici un extrait du code qui illustre l'implémentation du modèle avec rétropropagation et récompenses/pénalités :

```python
import numpy as np

# Initialisation des poids et biais
poids = np.random.randn(3, 1)  # Exemple de poids
biais = np.random.randn(1)  # Exemple de biais

def fonction_activation(x):
    return np.maximum(0, x)  # ReLU

def calcul_sortie(entree):
    sortie = np.dot(entree, poids) + biais  # Calcul pondéré
    return fonction_activation(sortie)

# Exemple de données d'entrée (position du véhicule)
entree = np.array([1, 2, 3])
